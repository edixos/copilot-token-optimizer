...
const CTO_BIN = path.resolve(__dirname, '../bin/cpto.js');

function writeCopilotInstructions(dir, content) {
  fs.mkdirSync(path.join(dir, '.github'), { recursive: true });
  fs.writeFileSync(path.join(dir, '.github', 'copilot-instructions.md'), content);
}
...
describe('unit — pure logic', () => {
  describe('buildAfterFiles()', () => {
    ...
    it('includes .github/copilot-instructions.md label', () => {
      const files = buildAfterFiles('2026-05-21');
      assert.ok(files.some(f => f.label === '.github/copilot-instructions.md'));
    });
  });
  ...
  describe('formatFileBreakdown()', () => {
    ...
    it('formats each file as a line', () => {
      const files = [{ label: '.github/copilot-instructions.md', tokens: 100 }, { label: 'README.md', tokens: 50 }];
      const lines = formatFileBreakdown(files);
      assert.equal(lines.length, 2);
      assert.ok(lines[0].includes('.github/copilot-instructions.md'));
      assert.ok(lines[1].includes('README.md'));
    });

    it('includes token counts', () => {
      const files = [{ label: '.github/copilot-instructions.md', tokens: 123 }];
      const lines = formatFileBreakdown(files);
  ...
  describe('formatMeasureReport()', () => {
    ...
    it('shows "Already initialized" when isInitialized is true', () => {
      ...
      const report = { before: 500, after, savings: 500 - after, files: [{ label: '.github/copilot-instructions.md', tokens: 500 }], afterFiles };
      const lines = formatMeasureReport(report, true, 'myproject');
      const text = lines.join('\n');
      assert.ok(text.includes('Already initialized'));
      assert.ok(!text.includes('cpto init'));
    });

    it('shows AFTER section and cpto init CTA when not initialized', () => {
      const afterFiles = buildAfterFiles('2026-05-21');
      const after = afterFiles.reduce((s, f) => s + f.tokens, 0);
...
describe('integration — filesystem', () => {
  ...
  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cpto-measure-'));
  });
  ...
  describe('buildReport()', () => {
    ...
    it('before is 0 for empty directory', async () => {
      const emptyDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cpto-measure-empty-'));
      try {
    ...
    it('savings is before minus after', async () => {
      const bigDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cpto-measure-big-'));
      try {
    ...
    it('includes file breakdown', async () => {
      const copilotDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cpto-measure-copilot-'));
      try {
        writeCopilotInstructions(copilotDir, '# Copilot');
        const report = await buildReport(copilotDir);
        ...
        fs.rmSync(copilotDir, { recursive: true });
      }
    });
  });

  describe('runMeasure() CTA', () => {
    it('shows cpto init CTA for uninitialized project', async () => {
      const uninitDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cpto-measure-uninit-'));
      try {
        ...
        assert.ok(output.includes('cpto init'), 'should suggest cpto init for uninitialized project');
    ...
    it('shows already-initialized message when .github/copilot-instructions.md exists', async () => {
      const initDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cpto-measure-init-'));
      try {
        writeCopilotInstructions(initDir, '# Copilot\n\nProject overview.');
        ...
        console.log = orig;
        const output = lines.join('\n');
        assert.ok(output.includes('Already initialized'), 'should show already-initialized for projects with .github/copilot-instructions.md');
        assert.ok(!output.includes('cpto init'), 'should not suggest cpto init when already initialized');
...
describe('e2e — subprocess', () => {
  it('exits 0 in empty directory', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cpto-measure-e2e-'));
    try {
