...
describe('integration — filesystem', () => {
  ...
  it('dry-run: does not write files when changes exist', async () => {
    const content = 'line\n\n\n\nother\n```javascript\ncode\n```\n';
    fs.writeFileSync(path.join(tmpDir, '.github/copilot-instructions.md'), content, 'utf8');
    ...
    assert.ok(!fs.existsSync(path.join(tmpDir, '.github/copilot-instructions.md.bak')), 'no backup in dry-run');
  });
...
describe('e2e — subprocess', () => {
  ...
  it('exits 0 and prints header with --dry-run', () => {
    ...
    assert.ok(
      result.stdout.includes('cpto compress'),
      `expected "cpto compress" in stdout:\n${result.stdout}`,
    );
  });

  it('does not modify .github/copilot-instructions.md with --dry-run', () => {
    const binPath = path.resolve(
      path.dirname(new URL(import.meta.url).pathname),
      '../bin/cpto.js',
    );
    ...
    const after = fs.readFileSync(path.join(tmpDir, '.github/copilot-instructions.md'), 'utf8');
    assert.strictEqual(after, before, '.github/copilot-instructions.md should be unchanged after --dry-run');
  });
});
