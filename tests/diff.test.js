...
const binPath = path.resolve(import.meta.dirname, '../bin/cpto.js');
...
describe('unit — pure logic', () => {
  ...
  it('formatDiffReport: returns array of strings', () => {
    ...
    const lines = formatDiffReport(d, '.github/copilot-instructions.md', '.github/copilot-instructions.md.bak');
  ...
  it('formatDiffReport: includes target label in output', () => {
    ...
    assert.ok(joined.includes('.github/copilot-instructions.md'));
  });

  it('formatDiffReport: shows "Saved" when current is smaller', () => {
    const d = computeDiff('short', 'much longer content here '.repeat(10));
    const joined = formatDiffReport(d, '.github/copilot-instructions.md', '.github/copilot-instructions.md.bak').join('\n');
...
describe('integration — filesystem', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cpto-diff-'));
  ...
  it('diffCommand runs without error when .bak exists', async () => {
    fs.writeFileSync(path.join(tmpDir, '.github/copilot-instructions.md'), 'shorter content');
    fs.writeFileSync(path.join(tmpDir, '.github/copilot-instructions.md.bak'), 'much longer content here '.repeat(20));
...
describe('e2e — subprocess', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cpto-diff-e2e-'));
    fs.mkdirSync(path.join(tmpDir, '.github'), { recursive: true });
  });
  ...
  it('exits 0 with no-backup message when no .bak file exists', () => {
    fs.writeFileSync(path.join(tmpDir, '.github/copilot-instructions.md'), 'some content');
    ...
    assert.ok(
      result.stdout.includes('No .github/copilot-instructions.md.bak found'),
      `expected "No .github/copilot-instructions.md.bak found" in stdout, got:\n${result.stdout}`
    );
  });
});
