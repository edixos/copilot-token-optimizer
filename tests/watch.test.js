...
describe('unit — pure logic', () => {
  ...
  describe('formatFileRow()', () => {
    it('includes rel filename', () => {
      const row = formatFileRow('.github/copilot-instructions.md', 100, 450);
      assert.ok(row.includes('.github/copilot-instructions.md'));
    });

    it('includes token count', () => {
      const row = formatFileRow('.github/copilot-instructions.md', 123, 450);
      assert.ok(row.includes('123'));
    });

    it('includes target when provided', () => {
      const row = formatFileRow('.github/copilot-instructions.md', 0, 450);
...
describe('integration — filesystem', () => {
  ...
  it('shows zero tokens when no files exist', () => {
    ...
    assert.ok(out.includes('.github/copilot-instructions.md'));
    assert.ok(out.includes('0 tokens') || out.match(/\s+0\s+tokens/), `expected 0 tokens: ${out.slice(0, 200)}`);
  });

  it('shows token count when .github/copilot-instructions.md exists', () => {
    fs.writeFileSync(path.join(tmpDir, '.github/copilot-instructions.md'), 'Hello world this is a test. '.repeat(20));
    const out = buildWatchDisplay(tmpDir);
    const match = out.match(/copilot-instructions\.md\s+(\d+)\s+tokens/);
  ...
  it('shows target in bar chart row', () => {
    const out = buildWatchDisplay(tmpDir);
    assert.ok(out.includes('target: 450'), `expected target in .github/copilot-instructions.md row: ${out}`);
  });

  it('no session writes section when write-log absent', () => {
    const freshDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cpto-watch-fresh-'));
    try {
  ...
  it('shows bar chart characters', () => {
    fs.writeFileSync(path.join(tmpDir, '.github/copilot-instructions.md'), 'x '.repeat(200));
    const out = buildWatchDisplay(tmpDir);
  ...
  it('token count updates when file changes', () => {
    fs.writeFileSync(path.join(tmpDir, '.github/copilot-instructions.md'), 'short');
    const out1 = buildWatchDisplay(tmpDir);
    fs.writeFileSync(path.join(tmpDir, '.github/copilot-instructions.md'), 'much longer content here '.repeat(50));
    const out2 = buildWatchDisplay(tmpDir);
    const m1 = out1.match(/copilot-instructions\.md\s+(\d+)\s+tokens/);
    const m2 = out2.match(/copilot-instructions\.md\s+(\d+)\s+tokens/);
...
describe('e2e — subprocess', () => {
  it('outputs token monitor header and exits on SIGTERM', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cpto-watch-e2e-'));
    try {
      let stdout = '';
