...
export function buildArchiveContent(date, content) {
  return `# Pruned from .github/copilot-instructions.md on ${date}\n\n${content}\n`;
}
...
function commitChanges(copilotMdPath, dir, original, accepted, options) {
  ...
  fs.writeFileSync(copilotMdPath, result, 'utf8');
  const afterTokens = countTokens(result);
  const beforeTokens = countTokens(original);
  console.log(chalk.green(`✓ Saved ${totalSaved} tokens — .github/copilot-instructions.md now ${afterTokens} tokens (was ${beforeTokens})`));
  console.log('');
}

function loadTargets(copilotMdPath) {
  if (!fs.existsSync(copilotMdPath)) {
    console.error(chalk.red('✗ .github/copilot-instructions.md not found. Run: cpto init'));
    process.exit(1);
  }
  console.log('');
  console.log(chalk.bold('cpto prune — scanning .github/copilot-instructions.md'));
  console.log('');
...
export async function pruneCommand(options) {
  ...
  const { original, targets } = loadTargets(copilotMdPath);
  if (targets.length === 0) {
    console.log(chalk.green('  Nothing to prune. .github/copilot-instructions.md looks clean.'));
    console.log('');
    return;
  }
  ...
  commitChanges(copilotMdPath, dir, original, accepted, options);
}
