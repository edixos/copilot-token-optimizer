...
export async function diffCommand(options = {}) {
  ...
  const targetFile = options.file || '.github/copilot-instructions.md';
  const targetPath = path.isAbsolute(targetFile) ? targetFile : path.join(dir, targetFile);
  const backupPath = targetPath + '.bak';
  const missingBackupLabel = path.isAbsolute(targetFile) ? backupPath : `${targetFile}.bak`;

  if (!fs.existsSync(backupPath)) {
    console.log('');
    console.log(chalk.yellow(`  No ${missingBackupLabel} found.`));
    console.log(`  Run ${chalk.cyan('cpto compress')} or ${chalk.cyan('cpto prune')} first (both create a .bak).`);
    console.log('');
    return;
  }
